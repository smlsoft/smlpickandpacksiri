 ...
 * @author BeamMary
 */
@WebServlet(name = "delApproved-Duplicate", urlPatterns = {"/delApprovedDuplicate"})
public class delApprovedDuplicate extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String __strQUERYgiztix = "/*[PickAndPack]Find Update Trans*/ "
                    + " select doc_no,ref_code from pp_trans where ref_code IN ( " +
"                    SELECT ref_code FROM (SELECT ref_code,doc_no as doc_pp,max(is_print) as is_print,COALESCE(CAST(lastedit_datetime AS VARCHAR), '') AS pp_last_edit  " +
...
"                    FROM pp_trans WHERE status=0 and is_close != 1 group by ref_code,pp_last_edit,doc_no) AS temp1 " +
"                    WHERE pp_last_edit <> ic_last_edit and is_print = 1 ORDER BY ref_code);";
            ResultSet __rsDataGetDocNo;
            System.out.println(__strQUERYgiztix);
            __stmtGetICTransList = conn.prepareStatement(__strQUERYgiztix);
            __rsDataGetDocNo = __stmtGetICTransList.executeQuery();
            while (__rsDataGetDocNo.next()) {
                ...
                // ### INSERT LOG
                String __strQUERY = "INSERT INTO pp_trans_log (doc_no,ref_code,trans_flag,user_code,action) VALUES ('" + __rsDataGetDocNo.getString("doc_no") + "','" + __rsDataGetDocNo.getString("ref_code") + "','" + "44" + "','" + __strUserCode + "','" + __strStatus + "')";
                ...
                __strQUERY = "DELETE FROM pp_trans WHERE ref_code='" + __rsDataGetDocNo.getString("ref_code") + "' ";
                System.out.println(__strQUERY);
                ...
                __strQUERY = "DELETE FROM pp_trans_detail WHERE ref_code='" + __rsDataGetDocNo.getString("ref_code") + "' ";
                System.out.println(__strQUERY);
            ...
            String __strOldDocNo2 = "";
            //System.out.println(__strQUERYgiztix);
            String doc_no = "";
            Integer i = 0;

            objResult.put("data", i);
            objResult.put("detail", doc_no);
